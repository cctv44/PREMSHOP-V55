  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const purchase = await tx.purchase.create({
      data: {
        ...
        credentials: `AUTO-GEN-${Math.random().toString(36).substring(7).toUpperCase()}`,
      },
    });

    await tx.auditLog.create({
        data: { userId: user.id, action: 'PURCHASE', details: { productId: product.id, price: product.price } }
    });
